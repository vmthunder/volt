...
"""Voltracker exception subclasses"""
...
class VoltrackerException(Exception):
    """
    ...
    def __init__(self, message=None, *args, **kwargs):
        ...
        try:
            ...
            if _FATAL_EXCEPTION_FORMAT_ERRORS:
                ...
                pass
        self.msg = message
        super(VoltrackerException, self).__init__(message)
...
class MissingCredentialError(VoltrackerException):
    message = _("Missing required credential: %(required)s")


class BadAuthStrategy(VoltrackerException):
    message = _("Incorrect auth strategy, expected \"%(expected)s\" but "
                "received \"%(received)s\"")


class NotFound(VoltrackerException):
    message = _("An object with the specified identifier was not found.")


class UnknownScheme(VoltrackerException):
    message = _("Unknown scheme '%(scheme)s' found in URI")


class BadStoreUri(VoltrackerException):
    message = _("The Store URI was malformed.")


class Duplicate(VoltrackerException):
    message = _("An object with the same identifier already exists.")


class Conflict(VoltrackerException):
    message = _("An object with the same identifier is currently being "
                "operated on.")


class StorageFull(VoltrackerException):
    message = _("There is not enough disk space on the image storage media.")


class StorageQuotaFull(VoltrackerException):
    message = _("The size of the data %(image_size)s will exceed the limit. "
                "%(remaining)s bytes remaining.")


class StorageWriteDenied(VoltrackerException):
    message = _("Permission to write image storage media denied.")


class AuthBadRequest(VoltrackerException):
    message = _("Connect error/bad request to Auth service at URL %(url)s.")


class AuthUrlNotFound(VoltrackerException):
    message = _("Auth service at URL %(url)s not found.")


class AuthorizationFailure(VoltrackerException):
    message = _("Authorization failed.")


class NotAuthenticated(VoltrackerException):
    message = _("You are not authenticated.")


class Forbidden(VoltrackerException):
...
class Invalid(VoltrackerException):
...
class AuthorizationRedirect(VoltrackerException):
    message = _("Redirecting to %(uri)s for authorization.")


class ClientConnectionError(VoltrackerException):
    message = _("There was an error connecting to a server")


class ClientConfigurationError(VoltrackerException):
    message = _("There was an error configuring the client.")


class MultipleChoices(VoltrackerException):
    message = _("The request returned a 302 Multiple Choices. This generally "
                "means that you have not included a version indicator in a "
                "request URI.\n\nThe body of response returned:\n%(body)s")


class LimitExceeded(VoltrackerException):
...
class ServiceUnavailable(VoltrackerException):
...
class ServerError(VoltrackerException):
    message = _("The request returned 500 Internal Server Error.")


class UnexpectedStatus(VoltrackerException):
    message = _("The request returned an unexpected status: %(status)s."
                "\n\nThe response body:\n%(body)s")


class InvalidContentType(VoltrackerException):
    message = _("Invalid content type %(content_type)s")


class BadRegistryConnectionConfiguration(VoltrackerException):
    message = _("Registry was not configured correctly on API server. "
                "Reason: %(reason)s")


class BadStoreConfiguration(VoltrackerException):
    message = _("Store %(store_name)s could not be configured correctly. "
                "Reason: %(reason)s")


class BadDriverConfiguration(VoltrackerException):
    message = _("Driver %(driver_name)s could not be configured correctly. "
                "Reason: %(reason)s")


class StoreDeleteNotSupported(VoltrackerException):
    message = _("Deleting images from this store is not supported.")


class StoreGetNotSupported(VoltrackerException):
    message = _("Getting images from this store is not supported.")


class StoreAddNotSupported(VoltrackerException):
    message = _("Adding images to this store is not supported.")


class StoreAddDisabled(VoltrackerException):
    message = _("Configuration for store failed. Adding images to this "
                "store is disabled.")


class MaxRedirectsExceeded(VoltrackerException):
    message = _("Maximum redirects (%(redirects)s) was exceeded.")


class InvalidRedirect(VoltrackerException):
    message = _("Received invalid HTTP redirect.")


class NoServiceEndpoint(VoltrackerException):
    message = _("Response from Keystone does not contain a Glance endpoint.")


class RegionAmbiguity(VoltrackerException):
    message = _("Multiple 'image' service matches for region %(region)s. This "
                "generally means that a region is required and you have not "
                "supplied one.")


class WorkerCreationFailure(VoltrackerException):
    message = _("Server worker creation failed: %(reason)s.")


class SchemaLoadError(VoltrackerException):
    message = _("Unable to load schema: %(reason)s")


class InvalidObject(VoltrackerException):
    message = _("Provided object does not match schema "
                "'%(schema)s': %(reason)s")


class UnsupportedHeaderFeature(VoltrackerException):
    message = _("Provided header feature is unsupported: %(feature)s")


class InUseByStore(VoltrackerException):
    message = _("The image cannot be deleted because it is in use through "
                "the backend store outside of Glance.")


class ImageSizeLimitExceeded(VoltrackerException):
...
class RPCError(VoltrackerException):
    message = _("%(cls)s exception was raised in the last rpc call: %(val)s")


class TaskException(VoltrackerException):
...
class DuplicateItem(VoltrackerException):
